...
//for(int ti=0;ti<4;ti++){for(int tj=0;tj<4;tj++){std::cout << tM[ti][tj] << " ";}std::cout<<"\n";}std::cout<<"\n";
		...
		gubo.eyePos = CamPos;
		gubo.coneInOutDecayExp = glm::vec4(0.99f, 0.999f, 3.0f, 1.0f);
		...
		vkMapMemory(device, SkyBoxUniformBuffersMemory[is], 0,sizeof(ubo), 0, &data);
		memcpy(data, &ubo, sizeof(ubo));
